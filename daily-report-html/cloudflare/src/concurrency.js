...
function normalizeState(value) {
  ...
  return {
    active: Object.fromEntries(Object.entries(active).filter(([sessionId, entry]) => sessionId && Number.isFinite(Number(entry && entry.lastSeen))).map(([sessionId, entry]) => [sessionId, { lastSeen: Number(entry.lastSeen) }])),
    queue: queue.filter((entry) => entry && entry.sessionId && Number.isFinite(Number(entry.lastSeen))).map((entry) => ({ sessionId: String(entry.sessionId), joinedAt: Number(entry.joinedAt || entry.lastSeen), lastSeen: Number(entry.lastSeen) }))
  };
}
...
export class ConcurrencyLimiter {
  ...
  async apply(action, sessionId) {
    ...
    if (Object.keys(output.state.active).length || output.state.queue.length) {
      await this.state.storage.setAlarm(Date.now() + config.alarmIntervalMs);
    }
  ...
  async fetch(request) {
    ...
    const allowed = new Set(['enter', 'check', 'heartbeat', 'queue-heartbeat', 'release', 'leave-queue', 'aggregate']);
