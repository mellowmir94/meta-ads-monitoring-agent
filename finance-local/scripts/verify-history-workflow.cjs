...
function deductionHistoryRender() {`);
  ...
  try {
    ...
    await page.locator('[data-deduction-history-open]').click();
    assert.equal(await page.locator('.nav-button.active, [data-deduction-history-open].active').count(),1);
    await page.locator('.sidebar .nav-button[data-tab="finsight"]').click();
    assert.equal(await page.locator('.sidebar .nav-button[data-tab="finsight"].active').count(),1);
    assert.equal(await page.locator('[data-deduction-history-open].active').count(),0);
    await page.locator('[data-deduction-history-open]').click();
    assert.equal(await page.locator('[data-deduction-history-open].active').count(),1);
    assert.equal(await page.locator('.nav-button[data-tab].active').count(),0);
    assert.equal(await page.locator('#tab-commission').isHidden(),false);
    assert.equal(await page.locator('#deductionHistoryView').isHidden(),false);
    ...
    await view.locator('[data-deduction-history-batch-payment-select]').selectOption('2');
    assert.match(await view.innerText(),/5 payments after this/);
    assert.match(await view.innerText(),/Active requests/);
    assert.match(await view.innerText(),/4 saved deduction records across 2 requests/);
    assert.equal(await view.locator('[data-deduction-history-month]').isVisible(),false);
    await view.locator('[data-history-progress]').selectOption('completed');
    assert.equal(await view.locator('[data-deduction-history-body] > tr').count(),1);
    await view.locator('[data-history-progress]').selectOption('reconciliation');
    assert.equal(await view.locator('[data-deduction-history-body] > tr').count(),0);
    await view.locator('[data-history-progress]').selectOption('');
    ...
    await view.locator('[data-history-more] > summary').click();
    assert.equal(await view.locator('[data-deduction-history-month]').isVisible(),true);
    await page.screenshot({path:path.resolve(__dirname,'../preview-evidence/history-filters-desktop.png'),fullPage:false});
    await page.setViewportSize({width:390,height:844});
    await view.locator('[data-history-primary]').scrollIntoViewIfNeeded();
