...
(async()=>{
  ...
  class PDF {constructor(){this.internal={pageSize:{getWidth:()=>800,getHeight:()=>600},getNumberOfPages:()=>1};}addImage(){}setFont(){}setFontSize(){}setTextColor(){}text(value){pdfText.push(value);}autoTable(options){pdfTable=options;}output(){return null;} }
  const context=vm.createContext({financeDeliverDownload:(value,name)=>{if(value){blob=value;downloadName=name;}return true;},window:{JSZip,jspdf:{jsPDF:PDF},confirm:()=>true},document:{createElement:()=>({click(){downloadName=this.download;},remove(){}}),body:{append(){}}},URL:{createObjectURL:value=>(blob=value,'test'),revokeObjectURL(){}},financePdfLogo:async()=>logo,additionalJobsLoad:async()=>{},additionalJobsForScope:()=>Array.from({length:250},(_,i)=>({reference:'JOB-'+i,description:'Extra delivery '+i,amountCents:1250})),deductionMoney:c=>'RM '+(c/100).toFixed(2),formatNumber:n=>String(n),formatGrafanaTimestamp:s=>s,console});
