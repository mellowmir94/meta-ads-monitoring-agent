...
(async()=>{
  ...
  try{
    ...
    await page.addScriptTag({content:`const esc=value=>String(value).replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('"','&quot;');const deductionRiderKey=s=>String(s||'').toLowerCase();const deductionMoney=c=>'RM '+(c/100).toFixed(2);const deductionRows=()=>[{rider_name:'Test Rider'}];const deductionSingleRider=()=>({valid:true,rider:'Test Rider',key:'test rider'});const dates={start:'2026-09-14',end:'2026-09-20'};const auditCapture=()=>({scope:{dates}});const deductionStatementPayloadCache=new Map();const saved=[];async function deductionRequest(path,input){if(path==='/save-job'){let job=saved.find(j=>j.id===input.requestId);if(!job){job={...input,id:input.requestId,riderKey:'test rider',reference:'JOB-'+(saved.length+1),amountCents:Math.round(Number(input.amount)*100)};saved.push(job);}return {job};}return {jobs:saved,next:null};}function render(){document.getElementById('fixture').innerHTML=additionalJobsMarkup('test',deductionSingleRider(),dates,{loaded:true,grossCents:10000,statementAmounts:{epf:2500}});}`});
    ...
    await page.addScriptTag({content:'let financePdfLogoDataUrl=null; const FINANCE_PDF_LOGO_ASSET="assets/bateriku-finance-logo.png"; const versionedFinanceAsset=s=>s;'+extract('      async function financePdfLogo()','      function financeExportPanelTitle')});
