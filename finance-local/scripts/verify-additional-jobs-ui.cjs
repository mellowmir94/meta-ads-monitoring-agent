...
(async()=>{
  ...
  try{
    ...
    const positions=await page.evaluate(()=>Object.fromEntries(['add','save','reset'].map(key=>{const r=document.querySelector('[data-job-'+key+']').getBoundingClientRect();return [key,{x:r.x,y:r.y,right:r.right}];})));
    assert.ok(positions.save.x>positions.add.right+100,'Save should sit on the right, away from Add row');
    assert.ok(positions.reset.x>positions.save.right,'Clear should sit to the right of Save');
    assert.equal(positions.save.y,positions.reset.y,'Save and Clear should share one row');
