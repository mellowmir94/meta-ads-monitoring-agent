...
(async()=>{
 ...
 try{for(const {formats,retry,empty} of [{formats:['pdf','excel']},{formats:['pdf']},{formats:['excel']},{formats:['pdf','excel'],empty:true},{formats:['pdf','excel'],retry:true}]){
  ...
  await page.route('**/*',async route=>{const url=new URL(route.request().url());if(url.hostname!=='localhost')return route.abort();
   ...
   if(url.pathname.startsWith('/api/')){requests.push(url.pathname);const body=url.pathname==='/api/deductions/jobs'?{jobs,next:null}:url.pathname.includes('deductions')?{records:fixtures,next:null,actor:{role:'maker'},statementSentAt:'2026-09-21T00:00:00Z'}:{rows:empty?[]:[{order_id:'1',rider_name:'Test Rider',commission:350,quantity:1,created_at:'2026-09-15 12:00:00'}],rowCount:empty?0:1,truncated:false};return route.fulfill({contentType:'application/json',body:JSON.stringify(body)});}
