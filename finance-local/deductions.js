...
async function deductionHistoryDownloadBatch(groupId, button) {
  const format = button.dataset.statementFormat || 'pdf';
  ...
  try {
    const [, payload] = await Promise.all([ensureFinanceExportBundle(format), deductionCombinedPaymentStatementPayload(options, { start: filters.periodStart, end: filters.periodEnd })]);
    if (format === 'excel') { await downloadExcelTable(payload); if (feedback) feedback.textContent='Rider Excel statement downloaded. The PDF uses the same statement data.'; return; }
