...
function deductionEpfSchedule(month, holidays = new Set()) {
  return deductionFirstFourThursdayWeeks(month).map(week => {
...
async function deductionCreateBatch(id, selectedTypes, drafts = {}) {
  ...
  // Use the month containing the commission week's Thursday. This keeps a
  // Monday-Sunday week that crosses a month boundary in the operational month.
  const epfScheduleMonth = deductionAddDays(periodStart, 3).slice(0, 7); let holidayData = { dates: new Set(), warning: '', source: 'Government of Malaysia' };
  if (types.includes('epf')) {
    body.innerHTML = '<div class="deduction-feedback" role="status">Preparing the four-week EPF schedule…</div>';
    holidayData = await deductionPublicHolidays(Number(epfScheduleMonth.slice(0, 4)));
    if (!dialog.isConnected) return;
  }
  const epfSchedule = deductionEpfSchedule(epfScheduleMonth, holidayData.dates);
  const lineMarkup = types.map(type => {
    const plan = type === 'battery-tester' ? drafts[type]?.pricingMode || 'fixed-2' : type === 'epf' ? 'fixed-epf' : 'manual';
    const count = type === 'insurance' ? 2 : type === 'battery-tester' ? plan === 'fixed-7' ? 7 : plan === 'fixed-2' ? 2 : Math.max(1, Math.min(52, Number(drafts[type]?.installmentCount) || 3)) : type === 'manual' ? Math.max(1, Math.min(52, Number(drafts[type]?.installmentCount) || 1)) : type === 'epf' ? 4 : 1;
    ...
    if (type === 'epf') {
      const scheduleRows = epfSchedule.map((item, index) => '<label class="deduction-epf-date"><span>Week ' + (index + 1) + '</span><input type="date" data-line-epf-date value="' + esc(item.dueDate) + '" min="' + esc(item.week.start) + '" max="' + esc(item.week.end) + '" readonly required><small>' + esc(item.shifted ? 'Wednesday holiday · shifted to Friday' : 'Thursday') + '</small></label>').join('');
      return '<fieldset class="deduction-line deduction-epf-line" data-deduction-line="epf"><legend>EPF · RM100 monthly</legend><div class="deduction-line-fields"><label>Weekly deduction (RM)<input data-line-amount type="number" value="25.00" readonly required></label><label>Schedule month<input data-line-epf-month value="' + esc(epfScheduleMonth) + '" readonly></label><label>Total<input value="RM100.00 · 4 payments" readonly></label><input type="hidden" data-line-date value="' + esc(epfSchedule[0]?.dueDate || '') + '"><input type="hidden" data-line-count value="4"><input type="hidden" data-line-subtype value="EPF"></div><div class="deduction-schedule-head"><div><strong>Four weekly dates</strong><span>First four Thursday weeks only; no fifth-week deduction.</span></div><button type="button" data-epf-schedule-toggle aria-pressed="false">Unlock dates</button></div><div class="deduction-epf-schedule" data-epf-schedule-editor>' + scheduleRows + '</div><p class="deduction-line-rule" data-line-rule></p><p class="deduction-calendar-note' + (holidayData.warning ? ' is-warning' : '') + '">' + esc(holidayData.warning || 'National Wednesday holidays are checked against ' + holidayData.source + '. State holidays can be adjusted after unlocking.') + '</p><label class="deduction-line-reason">Reason / remarks <span>(optional)</span><textarea data-line-reason maxlength="2000" rows="2" placeholder="Add supporting notes if needed"></textarea></label></fieldset>';
    }
  ...
  const epfStatus = !types.includes('epf') ? '' : '<div class="deduction-eligibility' + (epfBlocked ? ' is-error' : '') + '" role="status" data-deduction-eligibility>' + (deductionFullWeek(periodStart, periodEnd) ? (gross >= 300 ? 'EPF eligible · ' + esc(formatMoney(gross)) + ' recorded for the selected week. This creates four RM25 deductions for ' + esc(epfScheduleMonth) + '.' : 'EPF requires at least RM300 commission in this filtered week.') : 'EPF requires exactly one full Monday–Sunday week. Change this table’s date range, then reopen this request.') + '</div>';
  ...
  form.querySelectorAll('[data-epf-schedule-toggle]').forEach(button => { button.onclick = () => {
    const line = button.closest('[data-deduction-line]'), unlock = button.getAttribute('aria-pressed') !== 'true';
    button.setAttribute('aria-pressed', String(unlock)); button.textContent = unlock ? 'Lock dates' : 'Unlock dates';
    line.querySelectorAll('[data-line-epf-date]').forEach(input => { input.readOnly = !unlock; });
  }; });
  form.querySelectorAll('[data-line-epf-date]').forEach(input => { input.onchange = () => {
    const line = input.closest('[data-deduction-line]'), first = line.querySelector('[data-line-epf-date]'); line.querySelector('[data-line-date]').value = first?.value || ''; updateLines();
  }; });
...
function deductionActionDialog(recordId, action) {
  ...
  if (action === 'view') {
    const editableEpf = record.type === 'epf' && progress.items.length === 4, scheduleMonth = record.epfScheduleMonth || record.deductionDate.slice(0, 7), epfWeeks = editableEpf ? deductionFirstFourThursdayWeeks(scheduleMonth) : [];
    const scheduleHead = editableEpf ? '<div class="deduction-schedule-head deduction-details-schedule-head"><div><h4>Monthly EPF schedule</h4><span>RM25 × 4 = RM100.00 · dates are locked by default.</span></div><div><button type="button" data-epf-detail-toggle aria-pressed="false">Unlock dates</button><button type="button" data-epf-detail-save hidden>Save and lock</button></div></div><p class="deduction-calendar-note">Edit only when Finance needs a state-holiday or operational adjustment. Week 5 is never included.</p><p class="deduction-feedback" data-epf-detail-feedback hidden></p>' : '<h4>Payment schedule</h4>';
    body.innerHTML = context + '<p class="deduction-note">' + esc(deductionPlanLabel(record)) + (record.reason ? ' · ' + esc(record.reason) : '') + '</p>' + (record.type === 'epf' && record.reportedWeeklyCommissionCents ? '<p class="deduction-eligibility">Recorded weekly commission ' + esc(deductionMoney(record.reportedWeeklyCommissionCents)) + ' · current filtered table at request time</p>' : '') + scheduleHead + '<div class="deduction-audit-table"><table><thead><tr><th>Payment</th><th>Deduction date</th><th>Amount</th><th>Status</th><th>Commission week</th></tr></thead><tbody>' + progress.items.map((item, index) => '<tr><td>' + (index + 1) + '</td><td>' + (editableEpf ? '<input class="deduction-detail-date" type="date" data-epf-detail-date value="' + esc(item.dueDate) + '" min="' + esc(epfWeeks[index].start) + '" max="' + esc(epfWeeks[index].end) + '" readonly required>' : esc(item.dueDate)) + '</td><td>' + esc(deductionMoney(deductionInstallmentAmount(record, item))) + '</td><td>' + esc(item.status) + '</td><td>' + esc(item.settlementPeriodStart ? item.settlementPeriodStart + ' — ' + item.settlementPeriodEnd : item.status === 'applied' ? 'Legacy · due-date reporting' : '—') + '</td></tr>').join('') + '</tbody></table></div><h4>Audit trail</h4><ol class="deduction-audit-list">' + (record.audit || []).map(item => '<li><div><strong>' + esc(item.action) + '</strong><span>' + esc(formatGrafanaTimestamp(item.at) || item.at) + '</span></div><p>' + esc(item.by || '—') + ' · ' + esc(item.reason || 'No additional remarks') + '</p>' + (item.amountCents != null ? '<small>' + esc(deductionMoney(item.amountCents)) + '</small>' : '') + '<details><summary>Full event details</summary><pre>' + esc(JSON.stringify(item, null, 2)) + '</pre></details></li>').join('') + '</ol>';
    if (editableEpf) {
      const toggle = body.querySelector('[data-epf-detail-toggle]'), save = body.querySelector('[data-epf-detail-save]'), inputs = [...body.querySelectorAll('[data-epf-detail-date]')], feedback = body.querySelector('[data-epf-detail-feedback]'), identity = deductionRequestIdentity(); let saving = false;
      const lock = locked => { inputs.forEach(input => { input.readOnly = locked; }); toggle.setAttribute('aria-pressed', String(!locked)); toggle.textContent = locked ? 'Unlock dates' : 'Lock dates'; save.hidden = locked; };
      toggle.onclick = () => lock(toggle.getAttribute('aria-pressed') === 'true');
      save.onclick = async () => {
        if (saving || inputs.some(input => !input.reportValidity())) return;
        const installmentDates = inputs.map(input => input.value), payload = { recordId: record.id, installmentDates }; saving = true; save.disabled = true; feedback.hidden = false; feedback.textContent = 'Saving the four EPF dates…';
        try {
          const saved = await deductionRequest('/update-schedule', { ...payload, requestId: identity(payload) });
          saved.installmentDates.forEach((value, index) => { inputs[index].value = value; }); lock(true); feedback.classList.remove('is-error'); feedback.textContent = 'EPF dates saved and locked.'; deductionState.loaded = false; await deductionLoad(); deductionHistoryRender();
