...
function deductionHistorySelectedPayment(group, type, today = deductionToday(), timing = '') {
  const options = deductionHistoryPaymentOptions(group, today).filter(option => option.record.type === type), selected = deductionHistoryPaymentSelections.get(group.id + '|' + type);
...
function deductionHistoryPaymentCell(group, type, today = deductionToday(), timing = '') {
  const options = deductionHistoryPaymentOptions(group, today).filter(option => option.record.type === type), selected = deductionHistorySelectedPayment(group, type, today, timing);
...
document.addEventListener('change', event => {
  if (event.target.matches?.('[data-deduction-history-status], [data-deduction-history-type], [data-deduction-history-due], [data-deduction-history-timing], [data-deduction-history-type-timing], [data-deduction-history-month]')) return deductionHistoryRender();
