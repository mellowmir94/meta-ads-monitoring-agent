...
function deductionHistorySelectedPayment(group, today = deductionToday(), timing = '') {
...
function deductionHistoryPaymentCell(group, today = deductionToday(), timing = '') {
  const options = deductionHistoryPaymentOptions(group, today), selected = deductionHistorySelectedPayment(group, today, timing);
...
function deductionHistoryRender() {
  ...
  view.querySelector('[data-deduction-history-body]').innerHTML = deductionState.loaded ? groups.map(group => {
    const p = group.progress, paymentCell = deductionHistoryPaymentCell(group, deductionToday(), filters.timing);
