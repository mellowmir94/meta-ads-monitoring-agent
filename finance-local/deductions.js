...
function deductionHistoryActions(record, checker) {
  ...
  if (status === 'pending') buttons += (checker ? '<button type="button" data-deduction-action="approve">Approve</button><button type="button" data-deduction-action="reject">Reject</button>' : '') + '<button type="button" data-deduction-action="cancel">Cancel</button>';
  if (checker && status === 'approved' && progress.remainingCents) buttons += '<button type="button" data-deduction-action="apply">Apply payment</button>';
