  ...
  if (action === 'view') {
    ...
    if (editable) {
      ...
      const syncRows = preferredDates => {
        const count = Math.max(1, Math.min(52, Number(countInput.value) || 1)), existing = preferredDates || dateInputs().map(input => input.value), dates = existing.slice(0, count);
        ...
        rows.innerHTML = dates.map((dueDate, index) => { const item = progress.items[index], settlement = item?.settlementPeriodStart ? item.settlementPeriodStart + ' — ' + item.settlementPeriodEnd : record.periodStart + ' — ' + record.periodEnd; return '<tr><td>' + (index + 1) + '</td><td><input class="deduction-detail-date" type="date" data-deduction-detail-date value="' + esc(dueDate) + '"' + (locked ? ' readonly' : '') + ' required></td><td>' + esc(Number.isInteger(cents) && cents > 0 ? deductionMoney(cents) : '—') + '</td><td>' + esc(item?.status || 'applied') + '</td><td>' + esc(settlement) + '</td></tr>'; }).join('');
      };
      syncRows(progress.items.map(item => item.dueDate));
      const snapshot = () => ({ dates: dateInputs().map(input => input.value), reason: reasonInput.value, amount: amountInput.value, count: countInput.value, plan: planInput?.value || record.pricingMode });
      let saving = false, original = snapshot();
      const lock = locked => { dateInputs().forEach(input => { input.readOnly = locked; }); reasonInput.readOnly = locked; planInput && (planInput.disabled = locked); amountInput.readOnly = locked || record.type === 'epf' || isBattery && planInput.value !== 'manual'; countInput.readOnly = locked || record.type === 'epf' || record.type === 'insurance' || isBattery && planInput.value !== 'manual'; toggle.setAttribute('aria-pressed', String(!locked)); toggle.textContent = locked ? 'Edit details' : 'Cancel edit'; save.hidden = locked; };
