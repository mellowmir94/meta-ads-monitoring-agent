...
async function deductionCreateBatch(id, selectedTypes, drafts = {}) {
  ...
  let holidayData = { dates: new Set(), warning: '', source: 'Government of Malaysia' };
  if (types.includes('epf')) {
    ...
    holidayData = {
      dates: new Set(calendars.flatMap(calendar => [...calendar.dates])),
      warning: calendars.map(calendar => calendar.warning).filter(Boolean).join(' '),
      source: calendars.map(calendar => calendar.source).find(Boolean) || 'Government of Malaysia'
    };
    if (!dialog.isConnected) return;
  }
  const epfSchedule = deductionEpfSchedule(epfAnchorDate, holidayData.dates);
  const lineMarkup = types.map(type => {
    ...
    if (type === 'epf') {
      const scheduleRows = epfSchedule.map((item, index) => '<label class="deduction-epf-date"><span>Payment ' + (index + 1) + '</span><input type="date" data-line-epf-date value="' + esc(item.dueDate) + '" readonly required><small>' + esc(item.shifted ? 'Wednesday holiday · shifted to Friday' : 'Thursday') + '</small></label>').join('');
      return '<fieldset class="deduction-line deduction-epf-line" data-deduction-line="epf"><legend>EPF · RM100 monthly</legend><div class="deduction-line-fields"><label>Weekly deduction (RM)<input data-line-amount type="number" value="25.00" readonly required></label><label>Schedule month<input data-line-epf-month type="month" value="' + esc(epfScheduleMonth) + '" readonly></label><label>Total<input value="RM100.00 · 4 payments" readonly></label><input type="hidden" data-line-date value="' + esc(epfSchedule[0]?.dueDate || '') + '"><input type="hidden" data-line-count value="4"><input type="hidden" data-line-subtype value="EPF"></div><div class="deduction-schedule-head"><div><strong>Four deduction dates</strong><span>Starts this week. Thursday by default; only a Wednesday-holiday week moves to Friday.</span></div><button type="button" data-epf-schedule-toggle aria-pressed="false">Unlock dates</button></div><div class="deduction-epf-schedule" data-epf-schedule-editor>' + scheduleRows + '</div><p class="deduction-line-rule" data-line-rule></p><p class="deduction-calendar-note' + (holidayData.warning ? ' is-warning' : '') + '">' + esc(holidayData.warning || 'National Wednesday holidays are checked against ' + holidayData.source + '. Unlock to move any date across weeks or months.') + '</p><label class="deduction-line-reason">Reason / remarks <span>(optional)</span><textarea data-line-reason maxlength="2000" rows="2" placeholder="Add supporting notes if needed"></textarea></label></fieldset>';
    }
