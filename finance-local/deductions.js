...
function deductionCreateBatch(id, selectedTypes, drafts = {}) {
  ...
  const epfStatus = !types.includes('epf') ? '' : '<div class="deduction-eligibility' + (epfBlocked ? ' is-error' : '') + '" role="status" data-deduction-eligibility>' + (deductionFullWeek(periodStart, periodEnd) ? (gross >= 300 ? 'EPF eligible · recorded table commission ' + esc(formatMoney(gross)) + '. Checker approval is still required.' : 'EPF requires at least RM300 commission in this filtered week.') : 'EPF requires exactly one full Monday–Sunday week. Change this table’s date range, then reopen this request.') + '</div>';
  body.innerHTML = '<form class="deduction-batch-form"><div class="deduction-request-context"><div><span>Rider</span><strong>' + esc(rider.rider) + '</strong></div><div><span>Commission period</span><strong>' + esc(periodStart + ' — ' + periodEnd) + '</strong></div><div><span>Filtered gross commission</span><strong>' + esc(formatMoney(gross)) + '</strong></div><div><span>Initial status</span><strong>Pending approval</strong></div></div><p class="deduction-note">Each selected type becomes its own auditable record. Only an applied payment reduces net commission.</p><label class="deduction-maker-field">Created by (self-declared)<input name="createdBy" maxlength="200" autocomplete="name" required placeholder="Finance staff name"></label>' + epfStatus + '<div class="deduction-lines">' + lineMarkup + '</div><div class="deduction-submit-bar"><div><strong data-deduction-request-total></strong><p role="alert" data-deduction-error></p></div><button type="submit"' + (epfBlocked ? ' disabled' : '') + '>Save ' + types.length + ' pending deduction' + (types.length === 1 ? '' : 's') + '</button></div></form>';
...
function deductionHistoryRender() {
  ...
  view.querySelector('[data-deduction-history-kpis]').innerHTML = [['Applied deductions', deductionMoney(totals.appliedCents)], ['Approved · not applied', deductionMoney(totals.approvedCents)], ['Remaining scheduled', deductionMoney(totals.remainingCents)], ['Reversed payments', deductionMoney(totals.reversedCents)], ['Pending deductions', formatNumber(totals.pendingCount)]].map(([label, value]) => '<article><span>' + esc(label) + '</span><strong>' + esc(deductionState.loaded ? value : '—') + '</strong></article>').join('');
  view.querySelector('[data-deduction-history-body]').innerHTML = deductionState.loaded ? groups.map(group => {
    const p = group.progress;
    const deductions = group.records.map(record => { const item = deductionHistoryProgress(record), total = item.items.reduce((sum, installment) => sum + deductionInstallmentAmount(record, installment), 0); return '<div class="deduction-batch-line"><div><strong>' + esc(deductionTypes[record.type] || record.type) + '</strong><small>' + esc(deductionPlanLabel(record)) + '</small>' + (record.reason ? '<small>Reason: ' + esc(record.reason) + '</small>' : '') + '</div><div><b>' + esc(deductionMoney(total)) + '</b><span class="deduction-history-status ' + esc(deductionStatus(record)) + '">' + esc(deductionStatus(record)) + '</span></div></div>'; }).join('');
    const actions = group.records.map(record => { const status = deductionStatus(record), item = deductionHistoryProgress(record); let buttons = '<button type="button" data-deduction-action="view">Details</button>'; if (status === 'pending') buttons += (checker ? '<button type="button" data-deduction-action="approve">Approve</button><button type="button" data-deduction-action="reject">Reject</button>' : '') + '<button type="button" data-deduction-action="cancel">Cancel</button>'; if (checker && status === 'approved' && item.remainingCents) buttons += '<button type="button" data-deduction-action="apply">Apply payment</button>'; if (checker && item.appliedCents) buttons += '<button type="button" data-deduction-action="reverse">Reverse payment</button>'; return '<div class="deduction-batch-action" data-deduction-record-id="' + esc(record.id) + '"><strong>' + esc(deductionTypes[record.type] || record.type) + '</strong><div>' + buttons + '</div></div>'; }).join('');
    return '<tr data-deduction-batch-id="' + esc(group.id) + '"><td><strong>' + esc(group.rider) + '</strong><small>' + group.records.length + ' deductions in one request</small><small>' + esc(group.references.join(' | ')) + '</small></td><td><div class="deduction-batch-lines">' + deductions + '</div></td><td>' + esc((group.periodStart || '-') + ' - ' + (group.periodEnd || '-')) + '</td><td><strong>' + p.paidCount + ' / ' + p.count + ' applied</strong><small' + (p.overdue ? ' class="deduction-overdue"' : '') + '>' + esc(p.nextDue ? (p.overdue ? 'Overdue: ' : 'Next: ') + p.nextDue : 'No scheduled payments') + '</small></td><td>' + esc(deductionMoney(p.appliedCents)) + '</td><td>' + esc(deductionMoney(p.remainingCents)) + '</td><td><span class="deduction-history-status ' + esc(group.status) + '">' + esc(group.status) + '</span></td><td>' + esc(group.createdBy || '-') + '<small>' + esc(formatGrafanaTimestamp(group.createdAt)) + '</small></td><td class="deduction-history-actions deduction-batch-actions">' + actions + '</td></tr>';
...
function deductionHistoryStatementPayload(records) {
  ...
  const pendingCents = active.reduce((sum, record) => sum + deductionHistoryProgress(record).remainingCents, 0);
  ...
  const typeRows = [...new Set(active.map(record => record.type))].map(type => {
    const matching = active.filter(record => record.type === type), cents = matching.reduce((sum, record) => sum + deductionHistoryProgress(record).items.reduce((itemSum, item) => itemSum + deductionInstallmentAmount(record, item), 0), 0), statuses = [...new Set(matching.map(deductionStatus))];
  ...
  return { ...payload, title: rider.rider, filename: filename + '-Commission-Statement', pdfFilename: filename + '.pdf', period: (start && end ? start + ' - ' + end : payload.period) + ' | only applied deductions reduce net commission', summary: { label: 'Net Commission', value: deductionMoney(netCents) }, footerRows: [footer, ...typeRows, row('APPLIED DEDUCTIONS', '- ' + deductionMoney(appliedCents)), ...(pendingCents ? [row('PENDING DEDUCTIONS', deductionMoney(pendingCents))] : []), row('NET COMMISSION', deductionMoney(netCents))] };
}
async function deductionHistoryExport(format) {
  ...
  if (format === 'pdf') { await downloadPdfTable(deductionHistoryStatementPayload(records)); return; }
  const columns = [['rider', 'Rider'], ['references', 'References'], ['deductions', 'Selected deductions'], ['period', 'Commission period'], ['progress', 'Payments applied'], ['applied', 'Applied'], ['remaining', 'Remaining'], ['status', 'Status'], ['createdBy', 'Created by'], ['schedule', 'Payment schedules']].map(([key, label]) => ({ key, label, value: row => row[key] }));
  const rows = deductionHistoryGroups(records).map(group => ({ rider: group.rider, references: group.references.join(' | '), deductions: group.records.map(record => (deductionTypes[record.type] || record.type) + ': ' + deductionPlanLabel(record) + (record.reason ? ' | ' + record.reason : '')).join(' | '), period: (group.periodStart || '') + ' - ' + (group.periodEnd || ''), progress: group.progress.paidCount + '/' + group.progress.count, applied: deductionMoney(group.progress.appliedCents), remaining: deductionMoney(group.progress.remainingCents), status: group.status, createdBy: group.createdBy + ' | ' + formatGrafanaTimestamp(group.createdAt), schedule: group.records.map(record => (deductionTypes[record.type] || record.type) + ': ' + deductionHistoryProgress(record).items.map(item => item.dueDate + ' ' + item.status + (item.paymentDate ? ' paid ' + item.paymentDate : '') + (item.settlementPeriodStart ? ' commission ' + item.settlementPeriodStart + '-' + item.settlementPeriodEnd : '')).join(', ')).join(' | ') }));
  await downloadExcelTable({ title: 'Rider Deduction History', panelTitle: 'Commission Rider', filename: 'Commission-Rider-Deduction-History', columns, rows, period: 'One row per request. Applied payments only reduce commission.' });
}
