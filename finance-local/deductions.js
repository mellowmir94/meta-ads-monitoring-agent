...
function deductionSelectedPreview(draft, summary) {
  const amounts = { epf: 0, insurance: 0, 'battery-tester': 0, manual: 0 };
  for (const type of draft.selected) {
    const entered = type === 'epf' ? 25 : type === 'battery-tester' && draft.batteryPlan !== 'manual'
      ? draft.batteryPlan === 'fixed-7' ? 40 : 50 : Number(draft.amounts[type]);
    const count = type === 'manual' ? Number(draft.manualCount) : 1;
    if (Number.isFinite(entered) && entered > 0 && entered <= 1000000 && Number.isInteger(count) && count > 0) amounts[type] = Math.round(entered * 100 / count);
  }
  const approvedCents = Object.values(amounts).reduce((sum, cents) => sum + cents, 0);
  return { ...summary, amounts, statementAmounts: amounts, approvedCents, netCents: summary.grossCents + summary.additionalCents - approvedCents };
}
function deductionSummaryMarkup(dataRows, tableId) {
  ...
  const savedSummary = deductionSummaryForRows(dataRows, dates), draft = deductionDraftFor(tableId, rider, dates);
  const summary = deductionSelectedPreview(draft, savedSummary);
  const enabled = rider.valid && summary.loaded, disabled = enabled ? '' : ' disabled';
  ...
  const card = (type, controls, help) => '<article class="deduction-choice-card' + (selected(type) ? ' is-selected' : '') + '">' + choice(type) + controls + '<small>' + help + '</small><span class="deduction-card-applied">Selected payment <b>' + esc(enabled ? deductionMoney(summary.amounts[type]) : '—') + '</b></span></article>';
  ...
  return '<section class="commission-deduction-summary deduction-workspace" data-deduction-table="' + esc(tableId) + '" aria-label="Commission deduction calculation"><header class="deduction-workspace-head"><div><span class="deduction-eyebrow">Commission deduction formula</span><h3>' + esc(rider.valid ? rider.rider : 'Select one rider to prepare deductions') + '</h3><p>' + esc(String(dates.start || '').slice(0, 10) + ' — ' + String(dates.end || '').slice(0, 10)) + ' <span>· ' + formatNumber(dataRows.length) + ' filtered rows</span></p></div></header>' +
    ...
    '<div class="deduction-settlement-strip" data-preview-gross="' + summary.grossCents + '" data-preview-additional="' + summary.additionalCents + '"><div><span>Gross commission</span><strong>' + esc(deductionMoney(summary.grossCents)) + '</strong></div><span class="deduction-equation-sign" aria-hidden="true">+</span><div><span>Additional Jobs</span><strong>' + esc(deductionMoney(summary.additionalCents)) + '</strong></div><span class="deduction-equation-sign" aria-hidden="true">−</span><div><span>Total deductions</span><strong data-deduction-preview-total>' + esc(enabled ? deductionMoney(summary.approvedCents) : '—') + '</strong><small>' + esc(enabled ? 'Preview follows checked payments. Saved History is unchanged.' : 'Requires one rider and a complete register') + '</small></div><span class="deduction-equation-sign" aria-hidden="true">=</span><div class="commission-net-total' + (summary.netCents < 0 ? ' is-negative' : '') + '"><span>Net commission</span><strong data-deduction-preview-net>' + esc(enabled ? deductionMoney(summary.netCents) : '—') + '</strong></div></div>' +
...
function deductionUpdateInline(summary) {
  ...
  const strip = summary.querySelector('.deduction-settlement-strip');
  const selectedPreview = deductionSelectedPreview(draft, { grossCents: Number(strip?.dataset.previewGross || 0), additionalCents: Number(strip?.dataset.previewAdditional || 0) });
  if (deductionState.loaded && strip) {
    strip.querySelector('[data-deduction-preview-total]').textContent = deductionMoney(selectedPreview.approvedCents);
    strip.querySelector('[data-deduction-preview-net]').textContent = deductionMoney(selectedPreview.netCents);
    strip.querySelector('.commission-net-total').classList.toggle('is-negative', selectedPreview.netCents < 0);
    summary.querySelectorAll('.deduction-choice-card').forEach(card => {
      const type = card.querySelector('[data-deduction-inline-type]').value;
  ...
  const message = summary.querySelector('[data-deduction-inline-message]'); if (message && choices.length) message.textContent = valid ? 'Selected payment preview. Proceed saves the full plan (' + deductionMoney(total) + ') and opens History.' : 'Enter a valid amount for every selected deduction.';
