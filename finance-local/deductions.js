...
function deductionWorkflowPaymentMarkup(group, record, selected) {
  ...
  if (selected) {
    ...
    const badge = done ? 'Completed · ' + (selected.index + 1) + '/' + selected.count : single && selected.item.statementSentAt ? '1/1 · Statement sent · Awaiting reconciliation' : single ? '1/1 · Single payment · Awaiting statement' : selected.index === selected.count - 1 ? 'Final installment · awaiting completion · ' + (selected.index + 1) + '/' + selected.count : 'Installment ' + (selected.index + 1) + '/' + selected.count;
    const completionBlocked = !done && (selected.item.dueDate > deductionToday() || single && !selected.item.statementSentAt);
    const completionTitle = selected.item.dueDate > deductionToday() ? 'Available on or after the installment due date' : single && !selected.item.statementSentAt ? 'Download the PDF first so the statement is marked sent' : '';
...
function deductionHistoryDownloadCell(group, typeTimings = {}, today = deductionToday(), periodStart = '', periodEnd = '') {
  ...
  const readiness = sent ? '<span class="deduction-download-ready is-sent">✓ Sent to rider</span>' : upcoming.length ? '<span class="deduction-download-ready is-upcoming">Download now · stays Upcoming</span>' : '<span class="deduction-download-ready">Ready to download</span>';
  return '<td class="deduction-history-download-cell"><div class="deduction-history-download-item"><strong>' + esc(selected.length + ' selected payment' + (selected.length === 1 ? '' : 's')) + '</strong><small>' + esc(labels) + '</small>' + (upcoming.length ? '<small>' + esc(upcoming.length + ' upcoming payment' + (upcoming.length === 1 ? ' stays' : 's stay') + ' Upcoming until its scheduled date.') + '</small>' : '') + readiness + '<button type="button" data-deduction-history-batch-download>Download PDF</button></div></td>';
}
