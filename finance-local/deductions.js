...
const deductionDisplayStatus = value => ({ approved: 'scheduled', pending: 'scheduled' }[typeof value === 'string' ? value : deductionStatus(value)] || (typeof value === 'string' ? value : deductionStatus(value)));
...
function deductionSummaryMarkup(dataRows, tableId) {
  ...
  return '<section class="commission-deduction-summary deduction-workspace" data-deduction-table="' + esc(tableId) + '" aria-label="Commission deduction calculation"><header class="deduction-workspace-head"><div><span class="deduction-eyebrow">Commission deduction formula</span><h3>' + esc(rider.valid ? rider.rider : 'Select one rider to prepare deductions') + '</h3><p>' + esc(String(dates.start || '').slice(0, 10) + ' — ' + String(dates.end || '').slice(0, 10)) + ' <span>· ' + formatNumber(dataRows.length) + ' filtered rows</span></p></div></header>' +
    ...
    '<footer class="deduction-inline-actions"><button type="button" data-deduction-history-open>History</button><div><strong data-deduction-inline-preview>Request preview: —</strong><span data-deduction-inline-message>' + esc(enabled ? 'Choose one or more types. Saving proceeds directly to the active schedule.' : error || 'Waiting for the deduction register.') + '</span></div><strong class="deduction-inline-total">Total Deducted: ' + esc(enabled ? deductionMoney(summary.approvedCents) : '—') + '</strong><button type="button" data-deduction-inline-create' + (enabled && draft.selected.length ? '' : ' disabled') + '>Review deduction request</button></footer></section>';
}
...
async function deductionRefreshAfterSave(body, records, backupWarning = '') {
  ...
  body.innerHTML = '<section class="deduction-save-success" role="status"><span class="deduction-status-pill">Saved · scheduled</span><h4>Deduction schedule active</h4><p>Finance has proceeded with this request. Each payment will reduce commission when it is applied to its scheduled commission period.</p><ul>' + records.map(record => '<li><span>' + esc(deductionTypes[record.type] || record.type || 'Deduction') + '</span><strong>' + esc(record.reference || record.id) + '</strong></li>').join('') + '</ul><p data-deduction-refresh-message>Refreshing the register…</p><button type="button" data-deduction-done>Done</button></section>';
...
function deductionCreateBatch(id, selectedTypes, drafts = {}) {
  ...
  form.onsubmit = async event => {
    ...
    saving = true; submit.disabled = true; error.textContent = 'Activating deduction schedule…';
...
function deductionHistoryTotals(records) {
  return records.reduce((total, record) => { const p = deductionHistoryProgress(record); total.appliedCents += p.appliedCents; total.remainingCents += p.remainingCents; total.reversedCents += p.reversedCents;
    if (['pending', 'approved'].includes(deductionStatus(record))) { total.scheduledCount++; total.scheduledCents += p.remainingCents; } return total;
  }, { appliedCents: 0, remainingCents: 0, reversedCents: 0, scheduledCount: 0, scheduledCents: 0 });
}
...
function deductionHistoryActions(record, checker) {
  const status = deductionStatus(record), progress = deductionHistoryProgress(record); let buttons = '<button type="button" data-deduction-action="view">Details</button>';
  if (status === 'approved' && progress.remainingCents) buttons += '<button type="button" data-deduction-action="apply">Apply payment</button>';
...
function deductionHistoryRender() {
  ...
  view.querySelector('[data-deduction-history-kpis]').innerHTML = [['Applied deductions', deductionMoney(totals.appliedCents)], ['Scheduled deductions', deductionMoney(totals.scheduledCents)], ['Remaining scheduled', deductionMoney(totals.remainingCents)], ['Reversed payments', deductionMoney(totals.reversedCents)], ['Active schedules', formatNumber(totals.scheduledCount)]].map(([label, value]) => '<article><span>' + esc(label) + '</span><strong>' + esc(deductionState.loaded ? value : '—') + '</strong></article>').join('');
...
function deductionHistoryStatementPayload(records) {
  ...
  const scheduledCents = active.reduce((sum, record) => sum + deductionHistoryProgress(record).remainingCents, 0);
  ...
  return { ...payload, title: rider.rider, filename: filename + '-Commission-Statement', pdfFilename: filename + '.pdf', period: (start && end ? start + ' - ' + end : payload.period) + ' | only applied deductions reduce net commission', summary: { label: 'Net Commission', value: deductionMoney(netCents) }, footerRows: [footer, ...typeRows, row('APPLIED DEDUCTIONS', '- ' + deductionMoney(appliedCents)), ...(scheduledCents ? [row('SCHEDULED DEDUCTIONS', deductionMoney(scheduledCents))] : []), row('NET COMMISSION', deductionMoney(netCents))] };
}
async function deductionHistoryExport(format) {
  ...
  const columns = [['rider', 'Rider'], ['references', 'References'], ['epf', 'EPF'], ['insurance', 'Insurance'], ['battery', 'OBD / Battery Tester'], ['manual', 'Special Case'], ['period', 'Commission period'], ['progress', 'Payments applied'], ['applied', 'Applied'], ['remaining', 'Remaining'], ['status', 'Status'], ['createdBy', 'Created by'], ['schedule', 'Payment schedules']].map(([key, label]) => ({ key, label, value: row => row[key] }));
