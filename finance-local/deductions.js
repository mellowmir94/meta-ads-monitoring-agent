...
async function deductionPaymentStatementPayload(record, index) {
  ...
  const footerRow = (label, value) => columns.map((_column, columnIndex) => columnIndex === 0 ? label : columnIndex === commissionIndex ? value : '');
    ...
    if (editable) {
      ...
      const renderStatement = () => {
        ...
        statement.querySelector('[data-deduction-payment-download]').onclick = async event => {
          ...
          try { await ensureFinanceExportBundle('pdf'); await downloadPdfTable(await deductionPaymentStatementPayload(record, selectedIndex)); message.textContent = 'Weekly PDF downloaded with this payment only.'; }
