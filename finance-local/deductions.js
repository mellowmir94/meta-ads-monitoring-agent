...
function deductionActionDialog(recordId, action) {
  ...
  if (action === 'view') {
    const editable = progress.items.length > 0;
    const scheduleHead = '<div class="deduction-schedule-head deduction-details-schedule-head"><div><h4>' + (record.type === 'epf' ? 'Monthly EPF schedule' : 'Payment schedule') + '</h4><span>' + esc(deductionPlanLabel(record)) + ' · dates and remarks are locked by default.</span></div><div><button type="button" data-deduction-detail-toggle aria-pressed="false">Edit details</button><button type="button" data-deduction-detail-save hidden>Save and lock</button></div></div><p class="deduction-calendar-note">Edit payment dates or remarks here in History. Dates must remain unique and chronological.</p><p class="deduction-feedback" data-deduction-detail-feedback hidden></p>';
    body.innerHTML = context + (record.type === 'epf' && record.reportedWeeklyCommissionCents ? '<p class="deduction-eligibility">Recorded weekly commission ' + esc(deductionMoney(record.reportedWeeklyCommissionCents)) + ' · current filtered table at request time</p>' : '') + scheduleHead + '<div class="deduction-audit-table"><table><thead><tr><th>Payment</th><th>Deduction date</th><th>Amount</th><th>Status</th><th>Commission week</th></tr></thead><tbody>' + progress.items.map((item, index) => '<tr><td>' + (index + 1) + '</td><td><input class="deduction-detail-date" type="date" data-deduction-detail-date value="' + esc(item.dueDate) + '" readonly required></td><td>' + esc(deductionMoney(deductionInstallmentAmount(record, item))) + '</td><td>' + esc(item.status) + '</td><td>' + esc(item.settlementPeriodStart ? item.settlementPeriodStart + ' — ' + item.settlementPeriodEnd : item.status === 'applied' ? 'Legacy · due-date reporting' : '—') + '</td></tr>').join('') + '</tbody></table></div><label class="deduction-line-reason">Reason / remarks <span>(optional)</span><textarea data-deduction-detail-reason maxlength="2000" rows="2" readonly placeholder="Add supporting notes if needed">' + esc(record.reason || '') + '</textarea></label><h4>Audit trail</h4><ol class="deduction-audit-list">' + (record.audit || []).map(item => '<li><div><strong>' + esc(item.action) + '</strong><span>' + esc(formatGrafanaTimestamp(item.at) || item.at) + '</span></div><p>' + esc(item.by || '—') + ' · ' + esc(item.reason || 'No additional remarks') + '</p>' + (item.amountCents != null ? '<small>' + esc(deductionMoney(item.amountCents)) + '</small>' : '') + '<details><summary>Full event details</summary><pre>' + esc(JSON.stringify(item, null, 2)) + '</pre></details></li>').join('') + '</ol>';
    if (editable) {
      const toggle = body.querySelector('[data-deduction-detail-toggle]'), save = body.querySelector('[data-deduction-detail-save]'), inputs = [...body.querySelectorAll('[data-deduction-detail-date]')], reasonInput = body.querySelector('[data-deduction-detail-reason]'), feedback = body.querySelector('[data-deduction-detail-feedback]'), identity = deductionRequestIdentity(); let saving = false, originalDates = inputs.map(input => input.value), originalReason = reasonInput.value;
      const lock = locked => { inputs.forEach(input => { input.readOnly = locked; }); reasonInput.readOnly = locked; toggle.setAttribute('aria-pressed', String(!locked)); toggle.textContent = locked ? 'Edit details' : 'Cancel edit'; save.hidden = locked; };
      toggle.onclick = () => { const editing = toggle.getAttribute('aria-pressed') === 'true'; if (editing) { inputs.forEach((input, index) => { input.value = originalDates[index]; }); reasonInput.value = originalReason; lock(true); } else { originalDates = inputs.map(input => input.value); originalReason = reasonInput.value; lock(false); } };
      save.onclick = async () => {
        if (saving || inputs.some(input => !input.reportValidity())) return;
        const installmentDates = inputs.map(input => input.value), payload = { recordId: record.id, installmentDates, reason: reasonInput.value.trim() }; saving = true; save.disabled = true; feedback.hidden = false; feedback.textContent = 'Saving deduction details…';
        try {
          const saved = await deductionRequest('/update-details', { ...payload, requestId: identity(payload) });
          saved.installmentDates.forEach((value, index) => { inputs[index].value = value; }); reasonInput.value = saved.reason || ''; originalDates = inputs.map(input => input.value); originalReason = reasonInput.value; lock(true); feedback.classList.remove('is-error'); feedback.textContent = 'Details saved and locked.'; deductionState.loaded = false; await deductionLoad(); deductionHistoryRender();
