...
async function prepareRiderStatement(payload) {
  ...
  footerRows.splice(totalIndex<0?footerRows.length:totalIndex,0,...(jobs.length?[row('TOTAL ADDITIONAL JOBS','+ '+deductionMoney(extras))]:[]));
  ...
  const columns=payload.columns.map((column,i)=>({...column,value:record=>record.__additionalJob ? i===0?'ADDITIONAL JOB · '+record.reference+' · '+record.description : i===index?record.amountCents/100:'' : column.value(record)}));
  return {...payload,title:scope.rider,statementPrepared:true,sourceRowCount:payload.rows.length,columns,rows:[...payload.rows,...jobs.map(job=>({...job,__additionalJob:true}))],footerRows,exportSummaryRows:[],summary:{label:'Net Commission',value:deductionMoney(baseNet+extras)}};
}
...
async function downloadMasterRiderExcel(payload) {
  ...
  for(const footer of payload.footerRows){
    const label=String(footer[0]||'').toUpperCase(),style=label==='TOTAL DEDUCTIONS'?3:label==='NET COMMISSION'?5:2;
    const number=add(footer,style,label.startsWith('ADDITIONAL JOB')?40:24);
  ...
  const fills=['1D4ED8','174C3B','D9952F','FFFFFF','12754B'];
  zip.file('xl/styles.xml',`<styleSheet xmlns="${ns}spreadsheetml/2006/main"><fonts count="3"><font><sz val="11"/><name val="Aptos"/></font><font><b/><color rgb="FFFFFFFF"/><sz val="11"/><name val="Aptos"/></font><font><b/><color rgb="FF142135"/><sz val="11"/><name val="Aptos"/></font></fonts><fills count="7"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill>${fills.map(color=>`<fill><patternFill patternType="solid"><fgColor rgb="FF${color}"/><bgColor indexed="64"/></patternFill></fill>`).join('')}</fills><borders count="1"><border><left/><right/><top/><bottom/><diagonal/></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="6">${[0,2,3,4,5,6].map((fill,index)=>`<xf numFmtId="0" fontId="${index===0?0:[3,4].includes(index)?2:1}" fillId="${fill}" borderId="0" xfId="0" applyFont="1" applyFill="1" applyAlignment="1"><alignment vertical="center" wrapText="1"/></xf>`).join('')}</cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles></styleSheet>`);
