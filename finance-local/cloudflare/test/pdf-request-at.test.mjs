...
test('deduction totals and net commission use distinct PDF footer colors', () => {
  const pdf = html.slice(html.indexOf('      async function downloadPdfTable('), html.indexOf('      async function exportFinanceTable('));
  assert.match(pdf, /didParseCell: \(data\) => \{[\s\S]*?label === "APPLIED DEDUCTIONS" \|\| label === "TOTAL DEDUCTED"/u);
  assert.match(pdf, /data\.cell\.styles\.fillColor = \[217, 149, 47\]/u);
  assert.match(pdf, /label === "NET COMMISSION"[\s\S]*?data\.cell\.styles\.fillColor = \[18, 117, 75\]/u);
});
const source = name => {
...
test('PDF request_at matches the table date-time format for seconds and milliseconds', () => {
  ...
  for (const value of [Date.parse('2026-09-06T15:30:00Z'), Date.parse('2026-09-06T15:30:00Z') / 1000]) {
    assert.equal(context.financePdfCellValue(column, { value }), '2026-09-06 15:30:00');
    assert.equal(context.financePdfCellValue(column, { value: String(value) }), '2026-09-06 15:30:00');
  }
  assert.equal(context.financePdfCellValue(column, { value: '2026-09-06 12:34:56' }), '2026-09-06 12:34:56');
  assert.equal(context.financePdfCellValue(column, { value: null }), '');
  // Live Grafana API sample: order 3288497, dashboard timezone utc.
  assert.equal(context.financePdfCellValue(column, { value: 1788739140000 }), '2026-09-06 23:59:00');
