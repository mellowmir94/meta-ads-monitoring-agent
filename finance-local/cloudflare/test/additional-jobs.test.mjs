...
test('shared statement includes only matching rider/period jobs, adds once, cleans labels',async()=>{
  ...
  const master=await context.prepareRiderStatement(payload);assert.equal(master.summary.value,'RM 87.35');assert.equal(master.rows.length,2);assert.equal(master.columns[1].value(master.rows[1]),12.35);assert.equal(payload.rows.length,1);assert.equal(master.footerRows.filter(row=>row[0]==='NET COMMISSION').length,1);assert.ok(master.footerRows.some(row=>row[0]==='TOTAL DEDUCTIONS'));assert.ok(!JSON.stringify(master.footerRows).includes('(applied)'));
